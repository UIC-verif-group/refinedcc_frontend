...
(*  let cpp_config =
    let cpp_I =
      ...
      in
      let cpp_include = proj_include in
    ...
    cpp_I
  in *)
  (* Do the translation to CompCert, analyse, and generate our AST. *)
  ...
  let to_generate =
    let open C in
    let not_inlined def_or_decl =
      match def_or_decl.gdesc with
      | Gfundef(def) -> not (is_inlined def)
      | _                             -> false (* prototypes? *)
    in
    let fs = List.filter not_inlined coq_ast in
    let files = List.map (fun d -> proof_of_file (gd_name d)) fs in
...
(*let cpp_config =
  let build cpp_I cpp_include cpp_nostdinc cpp_D =
    {cpp_I; cpp_include; cpp_nostdinc; cpp_D}
  in
...
let cpp_cmd =
  let doc = "Print the result of the preprocessor to stdout." in
...
let _ =
  let cmds =
    [ init_cmd ; cpp_cmd ; check_cmd ; clean_cmd
